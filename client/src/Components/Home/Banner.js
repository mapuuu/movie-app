        ...
        <SwiperSlide key={index} className="relative rounded overflow-hidden">
          ...
          <div className="absolute linear-bg xl:pl-52 sm:pl-32 pl-8 top-0 bottom-0 right-0 left-0 flex flex-col justify-center lg:gap-8 md:gap-5 gap-4">
            ...
            <div className="flex gap-5 items-center">
              <Link
                to={`/movie/${movie?._id}`}
                className="bg-subMain hover:text-main transisions text-white px-8 py-3 rounded font-medium sm:text-sm text-xs"
              >
                Watch Now
              </Link>
              <button
                ...
                } hover:text-subMain transisions px-4 py-3 rounded text-sm bg-opacity-30`}
              >
                <FaHeart />
              </button>
            </div>
...
const Banner = ({ movies, isLoading }) => {
  const sameClass = ' w-full flex-colo xl:h-96 bg-dry lg:h-64';
